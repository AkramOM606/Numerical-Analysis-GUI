...
class Dialog(ctk.CTkToplevel):
    def __init__(self,title,mainlabel,textlabels,fgcolor):
        ...
        for i in range(len(textlabels)) :
            ...
            tmpFrame.pack()
            textLabel = ctk.CTkLabel(tmpFrame,text=textlabels[i],font=("Trebuchet MS Bold",16))
        ...
        buttonFrame.pack()
        SubmitButton = ctk.CTkButton(buttonFrame,text="OK",command=lambda : self.apply(len(textlabels)),width=50,font=("Trebuchet MS Bold",16))
        SubmitButton.grid(row=0,column=0,padx=5,pady=5)
        self.bind('<Return>', lambda event: SubmitButton.invoke())
        CancelButton = ctk.CTkButton(buttonFrame,text="Cancel",command=lambda : self.destroy(),width=50,font=("Trebuchet MS Bold",16))
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.home_button = ctk.CTkButton(self.header_sub_frame,width=40,height=40,corner_radius=8,text="",hover_color="red",command=lambda title="home":self.display_page(title),image=ctk.CTkImage(light_image=Image.open("./resources/images/home.png"),size=(40,40)))
        self.home_button.pack(side="right", anchor="ne", padx=5, pady=5)
        
        button_exit = ctk.CTkButton(self.header_sub_frame,width=40,height=40,corner_radius=8,text="",fg_color="red",hover_color="#FF0",command=self.quit,image=ctk.CTkImage(light_image=Image.open("./resources/images/exit.png"),size=(40,40)))
        button_exit.pack(side="right", anchor="ne", pady=5)

        self.home_text_var.set(value="Welcome to Numerical Analysis !")

        main_label = ctk.CTkLabel(self.header_frame,textvariable=self.home_text_var,font=("Trebuchet MS Bold",24))
        ...
        if button_title == "home":
            ...
            if (result) :
                ...
                for i in range(2):
                    subdataFrame.append([])
                    for j in range(5):
                        subdataFrame[i].append(ctk.CTkFrame(dataFrame,fg_color="transparent"))
                        subdataFrame[i][j].grid(row=i,column=j,sticky='nswe')
                
                for i in range(len(idata)):
                    tempLab = ctk.CTkLabel(subdataFrame[0][i],text=idata[i],font=("Trebuchet MS Bold",24))
                    tempLab.pack()
                ...
                for i in range(int(dim[0])):
                    AEntries.append([])
                    BEntries.append(ctk.CTkEntry(subdataFrame[1][4],font=("Trebuchet MS Bold",16)))
                    BEntries[i].grid(row=i,column=0,padx=5,pady=5)
                    for j in range(int(dim[1])):
                        if i == 0 :
                            subdataFrame[1][0].columnconfigure(j,weight=1)
                        AEntries[i].append(ctk.CTkEntry(subdataFrame[1][0],font=("Trebuchet MS Bold",16)))
                        AEntries[i][j].grid(row=i,column=j,padx=5,pady=5)
                
                tmpLabel = ctk.CTkLabel(subdataFrame[1][1],text="*",font=("Trebuchet MS Bold",24))
                tmpLabel.pack(expand="True")
                tmpLabel = ctk.CTkLabel(subdataFrame[1][2],text="?",font=("Trebuchet MS Bold",24))
                tmpLabel.pack(expand="True")
                tmpLabel = ctk.CTkLabel(subdataFrame[1][3],text="=",font=("Trebuchet MS Bold",24))
                tmpLabel.pack(expand="True")
                ...
                SolveButton = ctk.CTkButton(self.main_frame,text="Solve for X !",hover_color="red",font=("Trebuchet MS Bold",20),command = lambda : solveX(radio_var.get()))
                SolveButton.pack()
                ...
                def solveX(radiovar) :
                    ...
                    SoluLabel = ctk.CTkLabel(SolutionFrame,text = "X = ",font=("Trebuchet MS Bold",24))
            ...
            intLabel = ctk.CTkLabel(dataFrame,text="I = ",font=("Trebuchet MS Bold",32),compound="right",image=ctk.CTkImage(light_image=Image.open("./resources/images/integral.png"),size=(60,160)))
            intLabel.grid(row=0,column=0,rowspan=3)
            
            bEntry = ctk.CTkEntry(dataFrame,width=40,height=40,font=("Trebuchet MS Bold",20))
            bEntry.grid(row=0,column=1,padx=5,pady=5)
       
            aEntry = ctk.CTkEntry(dataFrame,width=40,height=40,font=("Trebuchet MS Bold",20))
            aEntry.grid(row=2,column=1,padx=5,pady=5)
       
            intEntry = ctk.CTkEntry(dataFrame,width=140,height=40,font=("Trebuchet MS Bold",20))
            ...
            def solveX(radiovar) :
                ...
                if radiovar == 0:
                    if (a<b):
                        x = np.arange(a,b+h,h)
                        f = ne.evaluate(exp)
                        SoluLabel = ctk.CTkLabel(SolutionFrame,text = f"I = {trapezes(a,b,f,h)}",font=("Trebuchet MS Bold",24))
                        SoluLabel.pack()
                    elif (a>b) :
                        x = np.arange(b,a+h,h)
                        f = ne.evaluate(exp)
                        SoluLabel = ctk.CTkLabel(SolutionFrame,text = f"I = {-trapezes(b,a,f,h)}",font=("Trebuchet MS Bold",24))
                        SoluLabel.pack()
                    else :
                        SoluLabel = ctk.CTkLabel(SolutionFrame,text = f"I = {0}",font=("Trebuchet MS Bold",24))
                        SoluLabel.pack()
                elif radiovar == 1 :
                    if (a<b):
                        x = np.arange(a,b+(h/2),h/2)
                        f = ne.evaluate(exp)
                        SoluLabel = ctk.CTkLabel(SolutionFrame,text = f"I = {simpson(a,b,f,h)}",font=("Trebuchet MS Bold",24))
                        SoluLabel.pack()
                    elif (a>b):
                        x = np.arange(b,a+(h/2),h/2)
                        f = ne.evaluate(exp)
                        SoluLabel = ctk.CTkLabel(SolutionFrame,text = f"I = {-simpson(b,a,f,h)}",font=("Trebuchet MS Bold",24))
                        SoluLabel.pack()
                    else :
                        SoluLabel = ctk.CTkLabel(SolutionFrame,text = f"I = {0}",font=("Trebuchet MS Bold",24))
                        SoluLabel.pack()
